{
    ...
    local roberta_version = 'roberta-large',

    model_name: 'sadga_roberta_bs=%(bs)d_lr=%(lr)s_blr=%(blr)s' % ({
        bs: batch_size,
        lr: '%0.1e' % lr,
        blr: '%0.1e' % bert_lr,
    }),
    ...
    model: {
        name: 'EncDec',
        encoder: {
            name: 'Encoder4Roberta',
            bert_token_type: true,
            roberta_version: roberta_version,
        ...
        encoder_preproc: {
            ...
            save_path: "preprocess_data/roberta/",
        },
        decoder_preproc: {
            grammar: {
                name: 'spider',
