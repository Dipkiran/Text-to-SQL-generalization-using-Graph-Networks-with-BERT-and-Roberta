...
class Trainer:
    ...
    def train(self, config, args):
        ...
        with self.init_random:
            if (hasattr(self.model.encoder, 'bert_model')):
                params = list(self.model.encoder.bert_model.parameters())
            else:
                params = list(self.model.encoder.roberta_model.parameters())
            assert len(params) > 0
            non_bert_params = []
            for name, _param in self.model.named_parameters():
                if "bert" not in name:
                    non_bert_params.append(_param)
            assert len(non_bert_params) + len(params) == len(list(self.model.parameters()))
