...
class EncoderPreproc4Roberta(EncoderPreproc):
    ...
    def __init__(
        ...
        self.tokenizer = RobertaTokenizer.from_pretrained(roberta_version)
        self.stanfordcore_nlp = StanfordCoreNLP(os.path.abspath(
            os.path.join(os.path.dirname(__file__), '../../../third_party/stanford-corenlp-full-2018-10-05')))
    ...
    def preprocess_item(self, item, validation_info):
        ...
        preproc_schema = self._preprocess_schema(item.schema)
        if self.compute_sc_link:
            question_roberta_tokens = Bertokens(question)
            sc_link = question_roberta_tokens.bert_schema_linking(
                preproc_schema.normalized_column_names,
                preproc_schema.normalized_table_names
            )
        else:
        ...
        if self.compute_cv_link:
            question_roberta_tokens = Bertokens(question)
            cv_link = compute_cell_value_linking(question_roberta_tokens.normalized_pieces, item.schema)
        else:
            cv_link = {"num_date_match": {}, "cell_match": {}}
...
@registry.register('encoder', 'Encoder4Roberta')
class Encoder4Roberta(torch.nn.Module):
    Preproc = EncoderPreproc4Roberta
    batched = True
    ...
    def __init__(
        ...
        self.bert_token_type = roberta_token_type
        self.base_enc_hidden_size = 1024 if roberta_version == "roberta-large" else 768
        ...
        self.tokenizer = self.preproc.tokenizer
        self.roberta_model.resize_token_embeddings(len(self.tokenizer))
    ...
    def forward(self, descs):
        ...
        batch_id_map = {}  # some long examples are not included
        for batch_idx, desc in enumerate(descs):
            qs = self.pad_single_sentence_for_roberta(desc['question'], cls=True)
            if self.use_column_type:
                cols = [self.pad_single_sentence_for_roberta(c, cls=False) for c in desc['columns']]
            else:
                cols = [self.pad_single_sentence_for_roberta(c[:-1], cls=False) for c in desc['columns']]
            tabs = [self.pad_single_sentence_for_roberta(t, cls=False) for t in desc['tables']]

            token_list = qs + [c for col in cols for c in col] + \
                         [t for tab in tabs for t in tab]
            assert self.check_roberta_seq(token_list)
            if len(token_list) > 512:
                long_seq_set.add(batch_idx)
                continue
        ...
        padded_token_lists, att_mask_lists, tok_type_lists = self.pad_sequence_for_roberta_batch(batch_token_lists)
        tokens_tensor = torch.LongTensor(padded_token_lists).to(self._device)
        att_masks_tensor = torch.LongTensor(att_mask_lists).to(self._device)

        if self.bert_token_type:
            tok_type_tensor = torch.LongTensor(tok_type_lists).to(self._device)
            roberta_output = self.roberta_model(tokens_tensor,
                                          attention_mask=att_masks_tensor, token_type_ids=tok_type_tensor)[0]
        else:
            roberta_output = self.roberta_model(tokens_tensor,
                                          attention_mask=att_masks_tensor)[0]

        enc_output = roberta_output
        column_pointer_maps = [
            {
                i: [i]
                for i in range(len(desc['columns']))
        ...
        for batch_idx, desc in enumerate(descs):
            ...
            if batch_idx in long_seq_set:
                ...
                roberta_batch_idx = batch_id_map[batch_idx]
                q_enc = enc_output[roberta_batch_idx][batch_id_to_retrieve_question[roberta_batch_idx]]
                col_enc = enc_output[roberta_batch_idx][batch_id_to_retrieve_column[roberta_batch_idx]]
                tab_enc = enc_output[roberta_batch_idx][batch_id_to_retrieve_table[roberta_batch_idx]]

                if self.summarize_header == "avg":
                    col_enc_2 = enc_output[roberta_batch_idx][batch_id_to_retrieve_column_2[roberta_batch_idx]]
                    tab_enc_2 = enc_output[roberta_batch_idx][batch_id_to_retrieve_table_2[roberta_batch_idx]]
    ...
    def encoder_long_seq(self, desc):
        ...
        enc_q = self._roberta_encode(qs)
        enc_col = self._roberta_encode(cols)
        enc_tab = self._roberta_encode(tabs)
        return enc_q, enc_col, enc_tab

    @DeprecationWarning
    def _roberta_encode(self, toks):
        if not isinstance(toks[0], list):  # encode question words
            indexed_tokens = self.tokenizer.convert_tokens_to_ids(toks)
            tokens_tensor = torch.tensor([indexed_tokens]).to(self._device)
            outputs = self.roberta_model(tokens_tensor)
            return outputs[0][0, 1:-1]  # remove [CLS] and [SEP]
        else:
            max_len = max([len(it) for it in toks])
            tok_ids = []
    ...
    def check_roberta_seq(self, toks):
        if toks[0] == self.tokenizer.cls_token and toks[-1] == self.tokenizer.sep_token:
            return True
        else:
            return False

    def pad_single_sentence_for_roberta(self, toks, cls=True):
        if cls:
            return [self.tokenizer.cls_token] + toks + [self.tokenizer.sep_token]
        else:
            return toks + [self.tokenizer.sep_token]

    def pad_sequence_for_roberta_batch(self, tokens_lists):
        pad_id = self.tokenizer.pad_token_id
        max_len = max([len(it) for it in tokens_lists])
        assert max_len <= 512
        toks_ids = []
